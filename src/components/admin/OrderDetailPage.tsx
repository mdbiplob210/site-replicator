...
import {
  Plus, Search, Calendar, AlertCircle, Truck,
  Download, Printer, RefreshCw, ChevronDown, Ban, Users,
  Trash2, Copy, X, ShoppingCart, ArrowLeft, Clock, CheckCircle2,
  PauseCircle, XCircle, Trash, Smartphone, BarChart3,
  MessageSquare, Loader2, Package, Globe, AlertTriangle, History,
  Hand, RotateCcw, CalendarClock, Phone, Pencil, Activity, FileText, ArrowRightLeft
} from "lucide-react";
import { useOrderItems, getStatusLabel, getStatusColor, type OrderStatus } from "@/hooks/useOrders";
...
function ActivityLogSection({ activityLogs, logFilterUser, setLogFilterUser, logFilterAction, setLogFilterAction, logFilterDate, setLogFilterDate }: {
  ...
  setLogFilterAction: (v: string) => void;
  logFilterDate: Date | undefined;
  setLogFilterDate: (v: Date | undefined) => void;
}) {
            ...
            <div className="flex-1 min-w-0">
              <p className="text-foreground">
                ...
                {log.action === "status_changed" && (
                  <span>স্ট্যাটাস পরিবর্তন: <Badge variant="outline" className="text-[9px] h-4">{log.old_value}</Badge> → <Badge variant="secondary" className="text-[9px] h-4">{log.new_value}</Badge></span>
                )}
