...
const BannerCarousel = () => {
  ...
  if (banners.length === 0) return <div className="w-full bg-muted" style={{ aspectRatio: "16/5", minHeight: "120px" }} role="img" aria-label="Banner placeholder" />;

  const banner = banners[current];

  const Wrapper = banner?.link_url
    ? ({ children }: { children: React.ReactNode }) => (
        <a href={banner.link_url} target="_blank" rel="noopener noreferrer" className="block" aria-label="Banner link">
          {children}
        </a>
      )
  ...
  return (
    <section className="relative w-full overflow-hidden bg-gray-100" style={{ aspectRatio: "16/5", minHeight: "120px" }} aria-label="Banner carousel" role="region">
      <Wrapper>
      ...
      {banners.length > 1 && (
        <>
          ...
          {/* Dots */}
          <div className="absolute bottom-2 sm:bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5" role="tablist" aria-label="Banner slides">
            {banners.map((_, i) => (
              <button
                key={i}
