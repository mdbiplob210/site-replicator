...
function closeCheckout(){document.getElementById('checkoutOverlay').style.display='none';document.body.style.overflow='';document.getElementById('formFields').style.display='';var success=document.getElementById('successMsg');if(success)success.style.display='none'}
document.getElementById('checkoutOverlay').addEventListener('click',function(e){if(e.target===this)closeCheckout()});
${summaryScript}
document.getElementById('checkoutForm').addEventListener('submit',function(e){e.preventDefault();e._templateHandled=true;var btn=document.getElementById('submitBtn');btn.disabled=true;btn.innerHTML='<span style="display:inline-block;width:18px;height:18px;border:2.5px solid #fff;border-top-color:transparent;border-radius:50%;animation:spin .6s linear infinite;margin-right:8px"></span> সাবমিট হচ্ছে...';var fd=new FormData(this);var q=parseInt(fd.get('quantity'))||1;var dcVal=q>=2?0:(parseInt(this.dataset.deliveryCharge)||${dc});var eid='eid_'+Math.random().toString(36).substr(2,9)+'_'+Date.now();var data={customer_name:fd.get('customer_name'),customer_phone:fd.get('customer_phone'),customer_address:fd.get('customer_address'),quantity:q,product_name:this.dataset.productName,product_code:this.dataset.productCode,unit_price:parseInt(this.dataset.unitPrice)||0,delivery_charge:dcVal,landing_page_slug:window._LP_SLUG||'',visitor_id:window._LP_VID||'',event_id:eid,event_url:window.location.href,fbp:window._lpTrack?window._lpTrack.getFbp():'',fbc:window._lpTrack?window._lpTrack.getFbc():''};data.total_amount=(data.unit_price*q)+dcVal;${useTiered ? 'var tp={1:'+t1+',2:'+t2+',3:'+t3+'};data.unit_price=Math.round((tp[q]||tp[1])/q);data.total_amount=(tp[q]||tp[1])+dcVal;' : ''}var baseUrl=window.SUPABASE_URL||'';fetch(baseUrl+'/functions/v1/submit-landing-order',{method:'POST',headers:{'Content-Type':'application/json','apikey':window.SUPABASE_ANON_KEY||''},body:JSON.stringify(data)}).then(function(r){return r.json()}).then(function(res){if(res&&(res.success||res.duplicate)){if(window._removePartial)window._removePartial();btn.innerHTML='✓ অর্ডার সফল!';btn.style.background='#10b981';var slug=window._LP_SLUG||data.landing_page_slug||'';var totalVal=data.total_amount||((data.unit_price||0)*q);var params=new URLSearchParams();params.set('order',res.order_number||'');params.set('eid',eid);params.set('value',String(totalVal));params.set('qty',String(q));if(data.product_name)params.set('product',data.product_name);if(data.product_code)params.set('code',data.product_code);if(res.order_id)params.set('oid',res.order_id);if(res.duplicate)params.set('duplicate','1');try{sessionStorage.setItem('_lp_purchase_success:'+eid,JSON.stringify({customer_name:data.customer_name,customer_phone:data.customer_phone,product_name:data.product_name||'',product_code:data.product_code||'',quantity:q,unit_price:data.unit_price||0,total_value:totalVal,order_id:res.order_id||'',order_number:res.order_number||'',duplicate:!!res.duplicate}))}catch(ex){}window.__lpOrderRedirecting=true;window.__lpPendingSuccessUrl=window.location.origin+'/lp/'+encodeURIComponent(slug)+'/success?'+params.toString();window.alert=function(){};var topWin=window;try{if(window.top&&window.top.location&&window.top.location.origin===window.location.origin)topWin=window.top;else if(window.parent&&window.parent!==window&&window.parent.location.origin===window.location.origin)topWin=window.parent}catch(ex){}setTimeout(function(){try{topWin.location.replace(window.__lpPendingSuccessUrl)}catch(ex){topWin.location.href=window.__lpPendingSuccessUrl}},0)}else{alert(res.error||'অর্ডার সাবমিট করতে সমস্যা হয়েছে');btn.disabled=false;btn.innerHTML='🛒 ${p.confirmButtonText}'}}).catch(function(err){alert('ত্রুটি: '+err.message);btn.disabled=false;btn.innerHTML='🛒 ${p.confirmButtonText}'})});
</script>
